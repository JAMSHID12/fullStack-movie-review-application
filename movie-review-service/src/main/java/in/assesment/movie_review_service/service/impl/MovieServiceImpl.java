...
//	private IReviewService reviewService;
	...
	@Override
	public ResponseDto<List<Movie>> getAllMoviesByUserId(long id) {
		System.out.println(id);
		List<Movie> movieList = movieRepository.findByUserId(id);
		if (!movieList.isEmpty()) {
			return new ResponseDto<>(movieList, "User fetched successfully", 200);
		} else {
			throw new NotFoundException("User not found with ID: " + id);
		}
	}
