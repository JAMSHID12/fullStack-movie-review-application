...
public interface IMovieRepository extends JpaRepository<Movie, Long> {
	...
	@Query(value = "SELECT DISTINCT m.* " + "FROM movies m " + "JOIN movies_genres mg ON m.id = mg.movie_id "
			...
			+ "AND (COALESCE(:minRating, NULL) IS NULL OR m.total_rating >= :minRating) ", nativeQuery = true)
	...
	@Query(value = "SELECT * from movies where user_id= :userId order by created_at desc;", nativeQuery = true)
	List<Movie> findByUserId(@Param("userId") long userId);

	@Query(value = "SELECT u.id AS userId, " + "m.id AS movieId, " + "m.description, " + "m.duration, "
			+ "m.movie_image_url AS movieImageUrl, " + "m.release_date AS releaseDate, " + "m.is_active AS isActive, "
			+ "m.title, " + "m.total_rated_count AS totalRatedCount, " + "m.total_rating AS totalRating, "
			+ "m.user_id AS movieCreatedUser, " + "u.name AS userName, " + "u.email " + "FROM movies m "
			+ "LEFT JOIN user_info u ON u.id = m.user_id " + "WHERE m.id = :movieId", nativeQuery = true)
	MovieByUser getMovieDetails(@Param("movieId") long movieId);
